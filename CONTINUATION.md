...
- **Tier guards left stale *inside function bodies* by the 0049 sweep** (found while planning Changes II; fixed in **0050**). 0049 rewrote the RLS policies and the masking views but not the guards hardcoded in plpgsql. `dashboard_financial_kpis` and `recompute_job_financials` still tested `auth_user_tier() < 2`, which under the new numbering meant: **Tier 2 received real P&L, charges and quoted totals from the dashboard RPC** — straight past the masking views — while **Tier 1 was denied and could not save any worksheet** (every save calls the recompute). Not reachable in practice at the time only because no tier-1/tier-2 account existed yet. **When changing the tier model, grep function bodies as well as policies:** `select proname from pg_proc where pg_get_functiondef(oid) ~ 'auth_user_tier\(\)\s*[<>=!]+\s*\d'` should return only `auth_can_act_on_job` and `guard_soft_delete`, which legitimately test `= 2`.
